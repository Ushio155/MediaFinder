...
let lastIndexMode = 'ps';
function refreshI18nUI() {
  setMonitorUI(lastMonitorState, lastIndexMode, (lastMonitorState && lastMonitorState.es) || null);
  renderLogDateLabel();
  els.logManageHint.innerHTML = t('日志按日期保存在 ') + '<code>%LOCALAPPDATA%\\MediaFinder\\logs</code>' + t('，可导出或删除以释放空间');
  if (lastLogs) renderLogs(lastLogs);
  if (els.searchMeta.textContent) doSearch();
}
...
function renderLogDateLabel() {
  els.logDate.textContent = (logDateStr === todayStr()) ? t('今天') : logDateStr;
  els.logDate.title = t('回到今天');
}
const logTypeLabel = { added: () => t('新增'), moved: () => t('移动'), gone: () => t('消失') };
function logDetailText(l) {
  if (l.type === 'moved') return t('旧: ') + l.old + t(' → 新: ') + l.new;
  if (l.type === 'added') return l.new || '';
  return l.old || t('未找到，可能已移入回收站或未监控目录');
}
...
function renderLogs(r) {
  renderLogDateLabel();
  const hintParts = [];
  if (r && r.lastCheck) hintParts.push(t('上次检查') + ' ' + r.lastCheck);
  hintParts.push(t('实时更新'));
  els.logHint.textContent = hintParts.join(' · ') + ' · ' + t('检测新增/移动/消失的媒体文件（可能移入回收站或未监控目录）');
  ...
  if (!logs.length) {
    ...
    li.textContent = allLogs.length ? t('该类型暂无事件') : (logDateStr === todayStr() ? t('暂无事件') : t('该日期暂无日志'));
  ...
  logs.forEach((l) => {
    ...
    badge.textContent = logTypeLabel[l.type] ? logTypeLabel[l.type]() : l.type;
    ...
    det.title = logDetailText(l) + t('（点击展开/收起）');
...
async function fetchLogs() {
  try {
    ...
    els.logHint.textContent = t('日志加载失败');
  }
}
...
async function api(path, body) {
  ...
  if (!res.ok) throw new Error(t('请求失败: HTTP ') + res.status);
  return res.json();
}
...
function fmtTime(iso) {
  ...
  if (diff < 60 * 60 * 1000) return Math.floor(diff / 60000) + t(' 分钟前');
  if (diff < 24 * 60 * 60 * 1000) return Math.floor(diff / 3600000) + t(' 小时前');
...
function setMonitorUI(state, indexMode, es) {
  ...
  if (indexMode === 'everything') {
    ...
    els.monitorBadge.textContent = '⚡ ' + t('实时索引');
    els.monitorBadge.className = 'badge badge-live';
    if (esState === 'down') {
      els.monitorBadge.textContent = '⚠ ' + t('索引异常');
      els.monitorBadge.className = 'badge badge-off';
      els.monitorText.textContent = t('Everything 不可用（进程未运行或权限不足），扫描/搜索可能返回空结果');
    } else if (esState === 'building') {
      els.monitorBadge.textContent = '⏳ ' + t('建库中');
      els.monitorBadge.className = 'badge badge-live';
      els.monitorText.textContent = t('Everything 正在构建索引，结果可能不完整，请稍候');
    } else {
      els.monitorText.textContent = t('Everything 实时索引已启用');
    }
    els.monitorToggle.checked = true;
    els.monitorToggle.disabled = true;
    $('monitorCardTitle').textContent = '⚡ ' + t('实时索引');
    $('monitorHint').textContent = t('基于 Everything 的 USN 日志实时追踪，文件新增 / 删除 / 重命名即时可见，无需手动刷新');
    return;
  }
  els.monitorToggle.disabled = false;
  const running = state && state.running;
  els.monitorToggle.checked = !!running;
  els.monitorText.textContent = running ? '● ' + t('自动刷新中') : '○ ' + t('已关闭');
  els.monitorBadge.textContent = running ? t('自动刷新开启') : t('自动刷新已关闭');
  els.monitorBadge.className = 'badge ' + (running ? 'badge-on' : 'badge-off');
  $('monitorCardTitle').textContent = '🔄 ' + t('自动刷新');
  $('monitorHint').textContent = t('开启后每几秒自动检查监控目录中的新文件并收录（低功耗）');
}
...
async function fetchStatus(silent) {
  try {
    ...
    els.indexTime.textContent = (s.index && s.index.live) ? t('实时更新') : fmtTime(s.index && s.index.modified);
    setMonitorUI(s.monitor, currentIndexMode, s.es);
    renderPaths(s.watchPaths || []);
    renderPrivacyDirs(s.privacyDirs || []);
    return s;
  } catch (e) {
    if (!silent) toast(t('无法连接服务器'), true);
    return null;
  }
}
...
async function doScan() {
  ...
  els.scanBtnLabel.innerHTML = '<span class="spinner"></span>' + t('扫描中…');
  els.scanHint.textContent = t('正在扫描配置目录…');
  try {
    ...
    if (!r.ok) {
      ...
      if (r.protected) {
        ...
        toast(t('扫描完成，共 ') + r.count + t(' 个媒体文件'));
      }
      doSearch();
    }
    fetchStatus(true);
  } catch (e) {
    els.scanHint.textContent = t('扫描失败');
    toast(t('扫描失败: ') + e.message, true);
  } finally {
    els.scanBtnLabel.textContent = '⟳ ' + t('重新扫描');
...
async function toggleMonitor() {
  ...
  try {
    ...
    toast(turningOn ? t('自动刷新已开启') : t('自动刷新已关闭2'));
  } catch (e) {
...
const typeLabel = { video: () => '🎥 ' + t('视频'), image: () => '🖼️ ' + t('图片'), audio: () => '🎵 ' + t('音频') };
...
function renderResults(items, kw) {
  els.results.innerHTML = '';
  if (!items.length) {
    showEmpty(kw ? t('未找到与 “') + kw + t('” 匹配的文件') : t('索引为空，请先点击左侧「重新扫描」'), '📭');
    ...
    groupItems.forEach((it) => {
      ...
      row.innerHTML = `
        ...
        <div class="result-actions">
          <button class="icon-btn" title="${t('打开文件')}" data-act="file">▶</button>
          <button class="icon-btn" title="${t('打开所在文件夹')}" data-act="folder">📂</button>
          <button class="icon-btn" title="${t('复制路径')}" data-act="copy">📋</button>
        </div>`;
...
async function loadBrowse(path) {
  ...
  try {
    ...
    if (!r.dirs || !r.dirs.length) {
      els.browseList.innerHTML = '<div class="browse-hint">' + t('此文件夹下没有子文件夹') + '</div>';
      return;
    }
    r.dirs.forEach((d) => {
    ...
    els.browseList.innerHTML = '<div class="browse-hint">' + t('无法读取该位置') + '</div>';
  }
}
async function doSuggest() {
  const kw = els.suggestKw.value.trim();
  if (!kw) { toast(t('请输入游戏或软件名称'), true); return; }
  els.suggestBtn.disabled = true;
  els.suggestList.innerHTML = '<div class="browse-hint">' + t('正在搜索…') + '</div>';
  try {
    const r = await api('/api/suggest', { keyword: kw });
    els.suggestList.innerHTML = '';
    if (!r.results || !r.results.length) {
      els.suggestList.innerHTML = `<div class="browse-hint">${t('未找到与“')}${escapeHtml(kw)}${t('”相关的文件夹，请尝试其它名称或手动浏览')}</div>`;
      return;
    }
    ...
    els.suggestList.innerHTML = '<div class="browse-hint">' + t('搜索失败') + '</div>';
  } finally {
...
async function removePath(path) {
  if (!confirm(t('确定移除该目录吗？') + '\n' + path)) return;
  try {
    const r = await api('/api/config/remove', { path });
    if (!r.ok) { toast(t('移除失败: ') + (r.error || ''), true); return; }
    toast(t('已移除目录'));
    await fetchStatus(true);
    doScan();
  } catch (e) {
...
async function renderLogManage() {
  els.logManageList.innerHTML = '<div class="browse-hint">' + t('加载中…') + '</div>';
  try {
    const r = await api('/api/logs/dates');
    const dates = r.dates || [];
    els.logManageList.innerHTML = '';
    if (!dates.length) {
      els.logManageList.innerHTML = '<div class="browse-hint">' + t('暂无历史日志') + '</div>';
      return;
    }
    dates.forEach((d) => {
      const item = document.createElement('div');
      item.className = 'log-manage-item';
      item.innerHTML = `<span class="lm-date">${escapeHtml(d.date)}</span><span class="lm-info">${d.events} ${t(' 条事件')} · ${fmtSize(d.size)}</span>`;
      const actions = document.createElement('div');
      actions.className = 'lm-actions';
      const expBtn = document.createElement('button');
      expBtn.className = 'btn btn-sm';
      expBtn.textContent = t('导出');
      ...
      delBtn.addEventListener('click', () => {
        if (!confirm(t('确定删除 ') + d.date + t(' 的日志吗？') + '\n' + t('此操作不可撤销。'))) return;
    ...
    els.logManageList.innerHTML = '<div class="browse-hint">' + t('加载失败') + '</div>';
  }
}
...
async function deleteAllLogs() {
  if (!confirm(t('确定删除全部历史日志吗？') + '\n' + t('此操作不可撤销。'))) return;
  try {
    const r = await api('/api/logs/delete-all', {});
    toast(t('已删除 ') + (r.deleted || 0) + t(' 个日志文件'));
    renderLogManage();
    logDateStr = todayStr();
    fetchLogs();
  } catch (e) {
...
els.refreshBtn.addEventListener('click', () => { toast(t('已刷新')); fetchStatus(); });
els.quitBtn.addEventListener('click', async () => {
  if (!confirm(t('确定要关闭 MediaFinder 服务器吗？'))) return;
  try {
    await api('/api/quit', {});
    toast(t('服务器已关闭'));
