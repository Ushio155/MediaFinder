...
- 实时更新：Everything 模式下搜索结果每 5 秒自动刷新，复制 / 移动 / 删除文件后无需手动点击；Everything 异常自动修复重启，索引实时有保障
- 类型筛选（视频/图片/音频）+ 时间筛选
- 游戏/软件素材自动分类（原神、鸣潮、Steam、剪映等 30+ 分类），分类名支持英文翻译（如「明日方舟终末地」→ Arknights EndField）
- 视频封面自动生成
- 深色/浅色主题
- 多语言界面：自动识别系统语言（中文系统显示中文，其他语言显示英文），右上角一键切换并记忆选择
- 活动日志：实时记录文件新增 / 移动 / 消失事件，支持按日期查看、导出与删除（防误报机制避免 Everything 波动产生假事件）
- 无 Everything 环境自动回退传统磁盘扫描模式
- 隐私保护：检测到微信/QQ 接收目录时在侧栏醒目提示（显示完整路径），由你决定是否纳入监控；纳入时自动排除聊天缓存（图片/视频/表情/缩略图/装扮等），仅收录接收的文件；"不再提示"选择永久记住
- 分类配额：每个分类最多显示 100 条，任何分类不会被海量结果挤占，分类与总数显示真实计数
...
首次运行 Everything 会建立全盘索引（约 1~3 分钟），之后秒级就绪。Everything 需要管理员权限读取 NTFS 变更日志，首次启动会请求 UAC 确认（允许后即可实时追踪文件变化）。
